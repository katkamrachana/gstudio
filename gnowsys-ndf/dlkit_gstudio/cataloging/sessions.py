...
from .objects import Catalog, CatalogList
...
class CatalogSession(abc_cataloging_sessions.CatalogSession, osid_sessions.OsidSession):
    ...
    def get_ids_by_catalog(self, catalog_id):
        ...
        For GSTUDIO, ids associated with the given catalog (PARENT-GROUP),
        can be considered as SUB-GROUPS.
    ...
    def get_ids_by_catalogs(self, catalog_ids):
        """Gets the list of ``Ids`` map to a list of ``Catalogs``.
        For GSTUDIO, ids associated with the given catalogs (PARENT-GROUPS)
    ...
    def get_catalog_ids_by_id(self, id_):
        ...
        For GSTUDIO, SUB-GROUPS' ids associated with the given catalog-id (PARENT-GROUP-ID).
    ...
    def get_catalogs_by_id(self, id_):
        ...
        For GSTUDIO, SUB-GROUPS associated with the given catalog-id (PARENT-GROUP-ID).
...
class CatalogLookupSession(abc_cataloging_sessions.CatalogLookupSession, osid_sessions.OsidSession):
    ...
    def get_catalog(self, catalog_id):
        ...
        *compliance: mandatory -- This method is must be implemented.*
        ...
        """
        return objects.Catalog(gstudio_node=node_collection.one({'_id': ObjectId(self._get_id(catalog_id, 'catalog').identifier)}))
    ...
    def get_catalogs_by_ids(self, catalog_ids):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        # return  # osid.cataloging.CatalogList
        catalog_list = []
        catalog_ids = [ObjectId(catalog_id.identifier) for catalog_id in catalog_ids]
        group_cur = node_collection.find({'_id': {'$in': catalog_ids}})
        catalog_list = [objects.Catalog(gstudio_node=each_grp) for each_grp in group_cur]
        return objects.CatalogList(catalog_list)
...
class CatalogAdminSession(abc_catalog_sessions.CatalogAdminSession, osid_sessions.OsidSession):
    ...
    def create_catalog(self, catalog_form):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        group_name = catalog_form._gstudio_map['name']
        status_obj_tuple = group_ins.create_group(group_name,**catalog_form._gstudio_map)
    ...
    def get_catalog_form_for_update(self, catalog_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        group_obj = Node.get_node_by_id(ObjectId(catalog_form.get_id().get_identifier()))

        cat_form = objects.CatalogForm(gstudio_node=group_obj, runtime=self._runtime, proxy=self._proxy)
    ...
    def update_catalog(self, catalog_form):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        """
        if not isinstance(Catalog, ABCCatalogForm):
            raise errors.InvalidArgument('argument type is not an Catalog')
        if not catalog_form.is_for_update():
            raise errors.InvalidArgument('the CatalogForm Catalog for update only, not create')
        try:
        ...
        group_ins = CreateGroup(req_obj)
        group_obj = Node.get_node_by_id(ObjectId(catalog_form.get_id().get_identifier()))
        group_obj = group_ins.get_group_fields(group_name, **catalog_form._gstudio_map)
        group_obj.save()
    ...
    def get_child_catalogs(self, catalog_id):
        ...
        *compliance: mandatory -- This method must be implemented.*
        ...
        return CatalogLookupSession(
            self._proxy,
            self._runtime).get_catalogs_by_ids(
                list(self.get_child_catalog_ids(repository_id)))
