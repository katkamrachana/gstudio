...
def create_or_update_counter(user_obj, group_id):
    ...
    ## INTERACTIONS ##
    counter_obj.comments_created = analytics_instance.get_total_comments_by_user()

    ## FILES ##
    # Get all files uploaded by user
    counter_obj.files_created = analytics_instance.get_user_files_count()
    # Get all comments posted on files uploaded by user
    counter_obj.comments_gained_on_files = analytics_instance.get_comments_counts_on_users_files()
    # Get all files on which the user has posted a comment
    counter_obj.comments_on_others_files = analytics_instance.get_other_files_commented_by_user_count()
    # Get all unique users who visited files uploaded by user
    counter_obj.visitors_on_files = analytics_instance.total_users_visited_my_files()
    counter_obj.visits_on_others_files = analytics_instance.get_others_files_read_count()
    counter_obj.avg_rating_gained_on_files = float(analytics_instance.get_ratings_received_on_user_files())

    ## NOTES ##
    # Get all notes/blog pages created by user
    counter_obj.notes_created = analytics_instance.get_user_notes_count()
    # Get all comments posted on notes created by user
    counter_obj.comments_gained_on_notes = analytics_instance.get_comments_counts_on_users_notes()
    # Get all notes on which user has posted comment
    counter_obj.comments_on_others_notes = analytics_instance.get_other_notes_commented_by_user_count()
    # Get all unique users who visited notes created by user
    counter_obj.visitors_on_notes = analytics_instance.total_users_read_my_notes()
    counter_obj.visits_on_others_notes = analytics_instance.get_others_notes_read_count()
    counter_obj.avg_rating_gained_on_notes = float(analytics_instance.get_ratings_received_on_user_notes())

    ## QUIZ ##
    # Get all quizitemevents attempted correctly by user
    counter_obj.correct_quizitems_attempted = analytics_instance.get_evaluated_quizitems_count(True,False)
    # Get all quizitemevents attempted incorrectly by user
    counter_obj.incorrect_quizitems_attempted = analytics_instance.get_evaluated_quizitems_count(False,True)
    # Get all quizitemevents attempted by user
    counter_obj.quizitems_attempted = analytics_instance.get_attempted_quizitems_count()
